...
export const CcdaToFhir = () => {
  ...
  const getPretifiedXML = (xmlString: string) => {
    let prettyXML = "";
    try {
      prettyXML = xmlFormat(xmlString);
    } catch (error) {
      setState((prevState) => ({
        ...prevState,
        isError: true,
        errorMessage: "Invalid C-CDA Document.",
      }));
      return xmlString;
    }
    setState((prevState) => ({
  ...
  const handleInputChange = useCallback(
    (value: string, viewUpdate: ViewUpdate) => {
      // Having defined as 'any' because the type definition is not available.
      // But this is not the best practice.
      const transaction: any = viewUpdate.transactions[0];
      const eventType: string = transaction.annotations[0].value;

      let xmlString = value;
      if (eventType == "input.paste") {
        xmlString = getPretifiedXML(value);
      }
      setState((prevState) => ({
        ...prevState,
        input: xmlString,
  ...
  const readFile = (fileInput?: string | ArrayBuffer | null) => {
    if (typeof fileInput == "string") {
      setState((prevState) => ({
        ...prevState,
        input: getPretifiedXML(fileInput),
      }));
    }
  };
  ...
  const inputEditor = (
    <CodeEditor
      ...
      placeholder="Paste or upload a C-CDA message here..."
      extensions={[xml()]}
      uploadEnabled
      readFile={readFile}
      clearEnabled
